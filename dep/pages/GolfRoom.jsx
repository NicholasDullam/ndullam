...
import cardBack from "../../public/assets/images/card_back.png";

import flipSoundFile from "../../public/assets/audio/flip.mp3";
import startSoundFile from "../../public/assets/audio/start.mp3";
...
const GolfRoom = ({ socket, room, setRoom, user_id }) => {
  const [user, setUser] = useState(
    room.users.find((item) => item.id === user_id)
  );
  ...
  useEffect(() => {
    ...
    setUser(
      room.users.find((item) => item.id === user_id) ||
        room.spectators.find((item) => item.id === user_id)
    );
  ...
  const flip = (room_id, user_id, card_id) => {
    if (user.hand.reduce((prev, curr) => prev + (curr.flipped ? 1 : 0), 0) >= 2)
      return;
    ...
    return (
      <div style={{ display: "flex", gap: "5px", flexDirection: "column" }}>
        ...
        <div
          style={{
            width:
              temp.id === room.turn || room.complete === temp.id
                ? "100%"
                : "0px",
            transition: "all 300ms ease",
            height: "5px",
            backgroundColor:
              room.complete === temp.id ? "rgba(74,222,128)" : "white",
            borderRadius: "10px",
            margin: "20px 0px",
          }}
        />
          ...
          {temp.hand.map((card, i) => {
            ...
            return (
              <div
                onClick={() =>
                  user.id === temp.id
                    ? replaceHandler(room.id, temp.id, i)
                    : null
                }
                className="hover:scale-[110%] transition-all duration-300 cursor-pointer"
                style={{
  ...
  const renderPlayers = () => {
    ...
    if (room.users.length > 2 && !(room.users.length % 2)) {
      ...
      for (let i = 0; i < remainder; i++) {
        ...
        if (i === Math.floor(remainder / 2))
          acrossPlayer = room.users[iterator];
      ...
      <div>
        ...
        >
          <div style={{ display: "flex", gap: "50px" }}>
            ...
            >
              <div
                onClick={() => drawCard(room.id, user_id, false)}
                className="hover:scale-[110%] transition-all duration-300 cursor-pointer"
                style={{
                  height: "70px",
                  width: "50px",
              ...
              >
                <img
                  src={cardBack}
                  style={{ userSelect: "none", height: "100%", width: "100%" }}
                />
              </div>
              <p
                style={{
            ...
            >
              <div
                onClick={() => discardHandler(room.id, user_id)}
                className="hover:scale-[110%] transition-all duration-300 cursor-pointer"
                style={{
                  height: "70px",
                  width: "50px",
                ...
                {room.discard.length ? (
                  <>
                    ...
                    >
                      ...
                      {
                        cardDisplays[
                          room.discard[room.discard.length - 1].value
                        ]
                      }{" "}
                    </p>
                    <p
              ...
              >
                <p style={{ textAlign: "center", userSelect: "none" }}>
                  ...
                  {room.turn === user?.id
                    ? "Your"
                    : `${
                        room.users.find((item) => item.id === room.turn)?.name
                      }'s`}{" "}
                  {room.complete ? "last" : null} turn{" "}
                </p>
                {room.users.find((item) => item.id === room.turn)
                  ?.profile_picture ? (
                  <img
                    src={
                      room.users.find((item) => item.id === room.turn)
        ...
        >
          ...
          >
            {room.users[userIndex]?.profile_picture ? (
              <img
                src={room.users[userIndex]?.profile_picture}
        ...
        {acrossPlayer ? (
          ...
          >
            ...
            >
              {acrossPlayer.profile_picture ? (
                <img
                  src={acrossPlayer.profile_picture}
                  style={{ height: "40px", width: "40px", borderRadius: "50%" }}
                />
              ) : null}
              <p style={{ textAlign: "center", userSelect: "none" }}>
                {" "}
                {acrossPlayer.name}'s hand{" "}
              </p>
            </div>
          ...
          {room.users.map((item) => {
            return (
              <p key={item.id} style={{}}>
                ...
                {item.name}:{" "}
                {room.complete === room.turn
                  ? `${item.points - item.history[item.history.length - 1]} + ${
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: "10px" }}>
        <button
          onClick={() => leaveRoom(room.id, user_id)}
          className="flex items-center transform rounded-3xl no-underline py-3 px-4 bg-black hover:text-black hover:bg-white hover:scale-110 shadow-md transition-all duration-300"
        >
          <p>Leave</p>
        </button>
        {room.admin === user_id && !room.active ? (
          <button
            onClick={() => start(room.id)}
            className="flex items-center transform rounded-3xl no-underline py-3 px-4 bg-black hover:text-black hover:bg-white hover:scale-110 shadow-md transition-all duration-300"
          >
            <p>Start</p>
          </button>
      ...
      {room.turn !== user?.id ? (
        ...
        >
          <div style={{ display: "flex" }}>
            ...
            <p style={{ color: "cyan" }}>
              {" "}
              {room.users.find((temp) => temp.id === room.turn)?.name}{" "}
            </p>
          </div>
        </div>
      ) : null}
      ...
      {room.active ? (
        <h4 className="text-m my-5"> Round {room.round} </h4>
      ) : null}
      {room.active ? (
        renderPlayers()
      ) : (
          ...
          {room.users.map((user) => {
            return (
              <p style={{ margin: "0px", color: "white" }}>
                {" "}
                {user.name} {room.admin === user.id ? <b>owner</b> : null}{" "}
              </p>
            );
          })}
        </div>
